...
function inferMappingFromDocument(document: ParsedDocument) {
  const headerRowIndex = inferBestImportHeaderRowIndex(document.sections[0]?.rows ?? [], 16);
...
function detectSplitMultilineCellRule(document: ParsedDocument) {
  const rows = document.sections[0]?.rows ?? [];
  const headerRowIndex = inferBestImportHeaderRowIndex(rows, 16);
  const header = rows[headerRowIndex] ?? [];
...
export function inferKeyValueExtractionConfig(document: ParsedDocument) {
...
export function buildHeuristicImportRule(document: ParsedDocument, fileType: SupportedImportFileType) {
  ...
  return {
    ...
    headerRowIndex,
    mapping,
    keyValueFields: keyValueConfig?.matchedFields ?? [],
  };
}
