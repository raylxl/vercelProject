...
export function isExamModeEnabled() {
  return process.env.UNIVERSAL_IMPORT_REQUIRE_AUTH !== "true" && process.env.UNIVERSAL_IMPORT_EXAM_MODE !== "false";
}
...
export async function authenticateUser(
  ...
  if (isExamModeEnabled() && !username && !password) {
    return {
      success: true,
...
export async function createSession(username = SYSTEM_USER_NAME) {
  if (isExamModeEnabled()) {
    return;
  }
